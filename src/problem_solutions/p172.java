...
public final class p172 implements EulerSolution
{
	public static void main(String[] args) {
		System.out.println(new p172().solve());
	}
    ...
    */
    ...
    private BigInteger countWays(List<Integer> freqs)
    {
        ...
        BigInteger ways = ProjectEulerLibrary.factorial(BASE);

        for (int x : histogram)
            ways = ways.divide(ProjectEulerLibrary.factorial(x));

        // Multinomial coefficient: LENGTH! / (freqs[0]! * freqs[1]! * ...)
        ways = ways.multiply(ProjectEulerLibrary.factorial(LENGTH));

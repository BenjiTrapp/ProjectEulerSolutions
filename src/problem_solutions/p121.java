...
public final class p121 implements EulerSolution
{
	public static void main(String[] args) {
		System.out.println(new p121().solve());
	}
	...
	public String solve()
	{
		...
		for (int i = TURNS / 2 + 1; i <= TURNS; i++)
			number = number.add(ways[TURNS][i]);

		denom = ProjectEulerLibrary.factorial(TURNS + 1);

		return denom.divide(number).toString();
	}
}

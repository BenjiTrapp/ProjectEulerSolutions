...
public final class p171 implements EulerSolution
{
	public static void main(String[] args) {
		System.out.println(new p171().solve());
	}
	
	private static final int LENGTH = 20;
	private static final int BASE = 10;
	private static final int MODULUS = ProjectEulerLibrary.pow(10, 9);  // Must be less than 2^31
    ...
    public String solve()
    {
        ...
        long[][] count = new long[LENGTH + 1][MAX_SQR_DIGIT_SUM + 1]; count[0][0] = 1;
        long s = 0;
        ...
        for (int i = 1; i <= LENGTH; i++)
        {
            for (int j = 0; j < BASE; j++)
            {
                ...
                {
                    sum[i][k + j * j] = (sum[i][k + j * j] + sum[i - 1][k] + ProjectEulerLibrary.powMod(BASE, i - 1, MODULUS) * j % MODULUS * count[i - 1][k]) % MODULUS;

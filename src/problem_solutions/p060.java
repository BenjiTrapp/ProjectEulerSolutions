...
public final class p060 implements EulerSolution
{
	...
	{
		System.out.println(new p060().solve());
	}


	private static final int PRIME_LIMIT = 100000;  // Arbitrary initial cutoff

	private int[] primes = ProjectEulerLibrary.listPrimes(PRIME_LIMIT);
	...
	private boolean isPrime(long x)
	{
		...
		{
			long end = ProjectEulerLibrary.sqrt(x);
			for (int p : primes)
			{
				if (p > end)
					break;

...
public final class p218 implements EulerSolution
{
	public static void main(String[] args) {
		System.out.println(new p218().solve());
	}
	...
	/* Alternative Code*/
	/*public  solveAlternativeWay()
	{
		// Find pythagorean triples (m,n,p) such that p <= 10^8
		long max = 100000000;
		for (long u = 1; true; u++)
		{
			for (long v = u+1; true; v+=2)
			{
				// gcd is defined in prob
				...
				// Since gcd(u,v)=1, gcd(uv, (u-v)(u+v)) = 1
				// Also m is even, n is odd. Therefore gcd(m,n) = 1.
				long a = 2 * m * n;
				long b = m * m - n * n;
				if (b < 0) b = -b;
				long c = m*m + n*n;

				// The triangle of interest is (a,b,c). c is guaranteed square.
				// We just need to assert that 84 doesn't divide ab/2.
				// m n (m - n) (m + n) is clearly divisible by 4 (m = 2 u v).
				// It's clearly divisible by 3 (if neither m nor n is then one
				// of m-n and m+n is). So the question is whether it's divisible
				// by 7.
				if (a % 7 != 0 && b % 7 != 0)
				{

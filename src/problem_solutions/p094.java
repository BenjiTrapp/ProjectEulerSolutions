...
public final class p094 implements EulerSolution
{
	public static void main(String[] args) {
		System.out.println(new p094().solve());
	}

	private static final int LIMIT = ProjectEulerLibrary.pow(10, 9);
	
	/**
	 * Intention behind the Solution:
	 * ==============================
	 *
	...
	public String solve()
	{
		...
		for (int s = 1; s * s <= (LIMIT + 1) / 3; s += 2)
		{
			...
			{
				if (ProjectEulerLibrary.gcd(s, t) == 1)
				{
					int a = s * t;
					int b = (s * s - t * t) / 2;
					int c = (s * s + t * t) / 2;

...
public final class p164 implements EulerSolution
{
	public static void main(String[] args) {
		System.out.println(new p164().solve());
	}
    ...
    public String solve()
    {
        ...
        BigInteger[][] ways = new BigInteger[DIGITS + CONSECUTIVE + 1][ProjectEulerLibrary.pow(BASE, CONSECUTIVE)];
            ...
            for (int prefix = 0; prefix < ways[digits].length; prefix++)
            {
                ...
                if (digitSum(prefix) <= MAX_SUM)
                    for (int nextDigit = 0; nextDigit < BASE; nextDigit++)
                        sum = sum.add(ways[digits - 1][prefix % ProjectEulerLibrary.pow(BASE, CONSECUTIVE - 1) * BASE + nextDigit]);
